"""Прогресс перебора комбинаций дат: события от `Planner` и их запись в
текстовые синки (stderr, файл прогона). Планнер остаётся чистым — он лишь
вызывает `ProgressReporter.start/emit`; кто и куда пишет строки, решает
граница (`cli.py`)."""
